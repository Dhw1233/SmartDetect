...
def get_arguments():
    ...
    parser.add_argument('--data', type=str, default='/home/gresham/persam/model/Personalize-SAM/data')
    parser.add_argument('--ckpt', type=str, default='/home/gresham/persam/model/Personalize-SAM/sam_vit_h_4b8939.pth')
...
def main():
    ...
    args = get_arguments()
    print("Args:", args)
    sys.path.append("/home/gresham/persam/")
    data_path = args.data
    ...
    for obj_name in os.listdir(data_path):
        persam_f(args, obj_name, data_path)


def persam_f(args, obj_name, data_path):
    raw_path = data_path + f"/{obj_name}/raw"
    img_list = os.listdir(raw_path)

    print("======> Load SAM" )
    if args.sam_type == 'vit_h':
        sam_type, sam_ckpt = 'vit_h', '/home/gresham/persam/model/Personalize-SAM/sam_vit_h_4b8939.pth'
    ...
    print("\n------------> Segment " + obj_name)
    for i in range(args.train_epoch_outside):
        output_path = data_path + f'/{obj_name}' + '/mask'
        mask_list = os.listdir(output_path)
        if not mask_list:
            return

        training_size = int(len(os.listdir(os.path.join(data_path, obj_name)))  * args.training_percentage)
        for ref_idx in range(1):
            # Path preparation
            images_path = data_path + f'/{obj_name}'+'/raw'
            masks_path = data_path + f'/{obj_name}' + '/mask'
            ref_image_path = os.path.join(images_path,  mask_list[0])
            ref_mask_path = os.path.join(masks_path,  mask_list[0])
            test_images_path = data_path + f'/{obj_name}'+'/raw'
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        history_mask = data_path + f'/{obj_name}' + '/mask/' + img_list[test_idx]
        if os.path.exists(history_mask):
            continue

        test_image_path = test_images_path + '/' + img_list[test_idx]
        ...
        for i in tqdm(range(args.max_objects)):
            ...
            mask_colors[final_mask, :] = np.array([[0, 0, 255]])

            mask_bool = mask_colors.sum(axis=2) == 255
        ...
        mask_output_path = os.path.join(output_path, img_list[test_idx])
